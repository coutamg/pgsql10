...
l1:
	...
	/*
	 ...
	 当txid=111的事务提交时，tuple_1就不再需要了，称为dead tuple。但是这个tuple
	 依然残留在页面上， 随着数据库的运行，这种死元组越来越多，它们会在VACUUM时最终被清理掉
	*/
 ...
 * See comments for struct HeapUpdateFailureData for additional info.
 * 
 * pg不会直接修改数据，而是将目标元组标记为删除，并插入一条新元组，同时修改
 * t_ctid 执行新版本元组
 * 
 * 参考 https://blog.csdn.net/Hehuyi_In/article/details/102868231
 */
HTSU_Result
			...
			HeapUpdateFailureData *hufd, LockTupleMode *lockmode)
{
	...
	/*
	 * If the page is already full, there is hardly any chance of doing a HOT(Heap Only Tuple)
	 ...
	 * 
	 * 如果页面已经满了，几乎没有机会在该页上进行HOT(堆仅元组)更新。如果先找索引列，然
	 * 后发现在同一页中缺少空间而拒绝HOT更新的行为是一种浪费。所以我们比较保守，只在页面还
	 * 没有满的时候才获取hot_attrs。因为我们已经在缓冲区上持有一个 pin，所以缓冲区没有机会
	 * 并发地被清理，即使这是可能的，在最坏的情况下，我们失去了做HOT更新的机会
	 */
	if (!PageIsFull(page))
	{
	 ...
	 * 
	 * 如果不更新任何键列，则可以获取较弱的锁类型。当我们同时运行外键检查时，这允许更多的并发性。
	 * 
	 * 注意，如果一个列在执行更新时被删除了，但是值最终是相同的，那么这个测试将失败，我们
	 * 将使用更强的锁。这是可以接受的;需要优化的重要情况是不操作键列的更新，而不是那些偶然
	 * 得到相同键值的更新。
	 */
	 ...
	 * use otid anymore.
	 * 
	 * 注意:在此之后，使用 oldtup 而不是 otid 来引用旧元组。Otid 很可能指向 newtup->t_self，
	 * 我们将用新元组的位置覆盖它，因此如果我们继续使用 Otid，将有很大的混淆风险。
	 */
...
l2:
	 ...
	 * 新的 tuple 也就是新插入元组的 tuple
	 * 
	 * 	t_xmin 被设置为 100，表示插入该元组的 txid
	 *	t_xmax 被设置为 0，因为该元组还未被更新或删除过
	 *	t_cid 被设置为 0，因为这是该事务的第一条命令（虽然又删又增，实际都是一条update操作的）
	 *	t_ctid 指向自身，被设置为（0,2），表示该元组位于 0 号 page 的第 2 个位置上
	 */
